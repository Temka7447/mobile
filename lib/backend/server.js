...
/**
 * POST /delivery
 * - Expects payload with items array [{ productId, name, price, quantity }, ...]
 * - Atomically decrements product quantities per-shop using positional operator
 * - Rolls back applied decrements if any item fails
 * - Saves a Delivery document via models/delivery.js
 */
app.post("/delivery", async (req, res) => {
  ...
  try {
    for (const it of items) {
      const rawProductId = it.productId?.toString?.() ?? it.productId;
      const qtyToRemove = Number(it.quantity) || 0;

      if (!rawProductId || qtyToRemove <= 0) {
        throw new Error(`Invalid item: ${JSON.stringify(it)}`);
      }

      // Accept both string IDs and 24-hex ObjectId strings.
      const productId = (/^[0-9a-fA-F]{24}$/.test(rawProductId)) ? mongoose.Types.ObjectId(rawProductId) : rawProductId;

      // Try to find and decrement the product quantity atomically.
      const updatedShop = await Shop.findOneAndUpdate(
        { 'products._id': productId, 'products.quantity': { $gte: qtyToRemove } },
        { $inc: { 'products.$.quantity': -qtyToRemove } },
        { new: true }
      );

      if (!updatedShop) {
        throw new Error(`Insufficient stock or product not found for productId=${rawProductId}`);
      }

      appliedUpdates.push({ productId: rawProductId, qty: qtyToRemove, shopId: updatedShop._id });
    }

    // All decrements succeeded: persist delivery document using the Delivery model
    ...
    return res.status(201).json({ success: true, message: 'Delivery created and stock updated', delivery: deliveryDoc });
  } catch (err) {
    console.error('Delivery processing error:', err.message || err);

    // rollback applied updates
    if (appliedUpdates.length > 0) {
      try {
        for (const a of appliedUpdates) {
          const pid = (/^[0-9a-fA-F]{24}$/.test(a.productId)) ? mongoose.Types.ObjectId(a.productId) : a.productId;
          await Shop.updateOne(
            { _id: a.shopId, 'products._id': pid },
            { $inc: { 'products.$.quantity': a.qty } }
          );
        }
      } catch (rbErr) {
        console.error('Failed to rollback updates after delivery error:', rbErr);
      }
    }

    return res.status(400).json({ success: false, message: err.message || 'Delivery failed' });
  }
});
